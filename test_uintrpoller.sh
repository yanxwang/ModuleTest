...
NUM_WORKERS=10
DURATION=60
OUTPUT_FILE="uintrpoller_test_results.csv"
CPU_LOG_FILE="uintrpoller_cpu0_all_runs.log"
THROUGHPUT_FILE="uintrpoller_throughput_all_runs.csv"
FREQ_LOG_FILE="uintrpoller_cpu0_freq_all_runs.log"
LATENCY_FILE="uintrpoller_latency_all_runs.csv"
...
for clients in "${CLIENTS_LIST[@]}"; do
    for qsize in "${QUEUE_SIZES[@]}"; do
        for run in $(seq 1 $RUNS); do
            echo "Running: clients=$clients, queue_size=$qsize, workers=$NUM_WORKERS, duration=${DURATION}s, run=$run"

            # Start mpstat for CPU 0 in background
            mpstat -P 0 1 > "cpu0_temp.log" &
            MPSTAT_PID=$!

            # Run uintrpoller test
            # Arguments: <num_clients> <queue_size> <num_workers> <duration_sec>
            ./uintrpoller "$clients" "$qsize" "$NUM_WORKERS" "$DURATION" > "test_output.log" 2>&1
...
echo "  - uintrpoller_throughput_vs_clients_bar.png"
echo "  - uintrpoller_cpu0_utilization_all_runs.png"
echo "  - uintrpoller_throughput_over_time.png"
echo "  - uintrpoller_latency_vs_clients.png"
